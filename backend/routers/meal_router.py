...
from models.dto_models import MealSearchFilters, MealSearchResult, SingleMealSearch
import requests
...
async def MealSearch(incomingSearches: SingleMealSearch, db: Session = Depends(getDb)):
    #Building the Spoonacular Request  
    spoonacularURL = f'https://api.spoonacular.com/recipes/{incomingSearches.id}/information?apiKey={config.api_key}'
    
    try:
        result = requests.get(spoonacularURL)
    ...
    try:
        NewMealResult = MealSearchResult(name=result['title'], id = str(result['id']), photolink=result['image'], calories='', protein='',carbs='', fat='')       
    except Exception as e:
        logger.error(str(e))
        raise HTTPException(            
    ...
    # An additional API call needs to be made to get the nutitional info
    ...
    try:
        ...
        for macros in nutritionalInfo.json()['nutrients']:
            if macros['name'] == 'Calories':
                NewMealResult.calories = macros['amount']
            elif macros['name'] == 'Protein':
                NewMealResult.protein = macros['amount']
            elif macros['name'] == 'Carbohydrates':
                NewMealResult.carbs = macros['amount']
            elif macros['name'] == 'Fat':
                NewMealResult.fat = macros['amount']
    except Exception as e:
        logger.error(str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def MealSearch(incomingSearches: MealSearchFilters, db: Session = Depends(getDb)):
    ...
    spoonacularURL = f'https://api.spoonacular.com/recipes/complexSearch?apiKey={config.api_key}&{searchFilters}&number={api_return_amount}'
...
"35.44"))
    ...
    returnMeals2.append(MealSearchResult(name="Test1", id = "656329", photolink="https://spoonacular.com/recipeImages/656329-312x231.jpg", calories="310.06", protein="15.57",carbs="13.58", fat=
"22.48"))
    returnMeals2.append(MealSearchResult(name="Test2", id = '680975', photolink="https://spoonacular.com/recipeImages/680975-312x231.jpg", calories="776.64", protein="34.73",carbs="74.61", fat=
"37.96"))
    returnMeals2.append(MealSearchResult(name="Test3", id = '663136', photolink="https://spoonacular.com/recipeImages/663136-312x231.jpg", calories="910.57", protein="44.84",carbs="105.96", fat=
"35.44"))
    
    if(incomingSearches.offset == '0'):
        exportMeals = returnMeals
    if(incomingSearches.offset == '3'):
        exportMeals = returnMeals2
